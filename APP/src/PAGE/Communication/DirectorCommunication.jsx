...
const DirectorCommunication = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const contactsRes = await axios.get('https://iqrab1.skoolific.com/api/chats/contacts/admin');
        dispatch(setContacts({ role: 'director', contacts: contactsRes.data }));

        // Fetch all requests for this director
        const requestsRes = await axios.get(`https://iqrab1.skoolific.com/api/chats/user/${directorId}`);
        ...
        try {
          const contactsRes = await axios.get('https://iqrab1.skoolific.com/api/chats/contacts');
          dispatch(setContacts({ role: 'director', contacts: contactsRes.data }));
        } catch (fallbackErr) {
  ...
  const handleSendResponse = async (requestId, questions) => {
    ...
    try {
      await axios.post(`https://iqrab1.skoolific.com/api/chats/requests/${requestId}/respond`, {
